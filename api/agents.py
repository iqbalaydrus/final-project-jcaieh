...
# Helper function untuk extract text dari PDF base64
def extract_text_from_pdf_base64(llm: ChatOpenAI, pdf_base64: str) -> str:
    """
    Text extracting from PDF base64 using multimodal langchain format
    """
    try:
        print("Extracting text from PDF using multimodal format...")
...
#cv_file_contents -> cv_pdf_base64
def chat(req: schema.ChatRequest, cv_pdf_base64: Optional[str]) -> Dict[str, Any]:
    """
    This is the main agent that routes questions to the appropriate specialist agent.
    PDF in base64 format might be sent to model for text extraction (including scanned PDFs).
    It returns a dictionary with the response and usage data.
    """
    ...
    if is_waiting_for_jd:
        ...
        # Extracting text from pdf base64
        with get_openai_callback() as extract_cb:
            cv_text = extract_text_from_pdf_base64(llm, cv_pdf_base64)
        
        if not cv_text:
            return {
                "content": "Error: Could not extract text from the uploaded CV. Please ensure it's a valid PDF file.",
                "agent_used": "ResumeAgent",
                "prompt_tokens": extract_cb.prompt_tokens,
                "completion_tokens": extract_cb.completion_tokens,
            }
        ...
        with get_openai_callback() as cb:
            analysis_result = resume_agent.run(cv_text=cv_text, job_description=job_description)

            if "error" in analysis_result:
                return {
                    "content": analysis_result["error"],
                    "agent_used": "ResumeAgent",
                    "prompt_tokens": extract_cb.prompt_tokens + cb.prompt_tokens,
                    "completion_tokens": extract_cb.completion_tokens + cb.completion_tokens,
                }
...
"""
    ...
    try:
        ...
        if "sql" in route.lower():
            ...
            # Extracting text if any
            cv_text = None
            extraction_tokens = {"prompt": 0, "completion": 0}
            
            if cv_pdf_base64:
                with get_openai_callback() as extract_cb:
                    cv_text = extract_text_from_pdf_base64(llm, cv_pdf_base64)
                    extraction_tokens["prompt"] = extract_cb.prompt_tokens
                    extraction_tokens["completion"] = extract_cb.completion_tokens
            ...
            with get_openai_callback() as consultation_cb:
                ...
                return {
                    ...
                    "prompt_tokens": extraction_tokens["prompt"] + consultation_cb.prompt_tokens,
                    "completion_tokens": extraction_tokens["completion"] + consultation_cb.completion_tokens,
                }
...
class ResumeAgent:
    ...
    def run(self, cv_text: str, job_description: str) -> Dict[str, any]:
        ...
        after_score = self._calculate_ats_score(rewritten_cv, jd_keywords)
